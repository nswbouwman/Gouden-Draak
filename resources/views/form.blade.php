<x-app-layout>
    <x-slider-with-border-tablet>
        <h1 class="text-xl font-bold">{{ __('form.thanks') }}</h1>
        <p class="mb-4 text-lg">{{ __('form.please_submit_form') }}</p>
        <form method="POST" action="{{ route('form.submit') }}">
            @csrf
            <div class="flex flex-col items-center mb-4">
                <label class="font-bold text-lg" for="experience">{{ __('form.experience') }}</label>
                <textarea class="w-[80%]" id="experience" name="experience" rows="3" class="form-control" required></textarea>
            </div>
            <div class="flex flex-col items-center mb-4">
                <label class="font-bold text-lg" for="atmosphere">{{ __('form.atmosphere') }}</label>
                <select class="w-[80%]" id="atmosphere" name="atmosphere" class="form-control" required>
                    <option value="">{{ __('form.make_a_choice') }}</option>
                    <option value="{{ __('form.perfect') }}">{{ __('form.perfect') }}</option>
                    <option value="{{ __('form.good') }}">{{ __('form.good') }}</option>
                    <option value="{{ __('form.fine') }}">{{ __('form.fine') }}</option>
                    <option value="{{ __('form.meh') }}">{{ __('form.meh') }}</option>
                    <option value="{{ __('form.bad') }}">{{ __('form.bad') }}</option>
                </select>
            </div>
            <div class="flex flex-col items-center mb-4">
                <label class="font-bold text-lg" for="food">{{ __('form.food') }}</label>
                <select class="w-[80%]" id="food" name="food" class="form-control" required>
                    <option value="">{{ __('form.make_a_choice') }}</option>
                    <option value="{{ __('form.perfect') }}">{{ __('form.perfect') }}</option>
                    <option value="{{ __('form.good') }}">{{ __('form.good') }}</option>
                    <option value="{{ __('form.fine') }}">{{ __('form.fine') }}</option>
            ...
            <div class="flex flex-col items-center mb-4">
                <label class="font-bold text-lg" for="service">{{ __('form.service') }}</label>
                <select class="w-[80%]" id="service" name="service" class="form-control" required>
                    <option value="">{{ __('form.make_a_choice') }}</option>
                    <option value="{{ __('form.perfect') }}">{{ __('form.perfect') }}</option>
                    <option value="{{ __('form.good') }}">{{ __('form.good') }}</option>
                    <option value="{{ __('form.fine') }}">{{ __('form.fine') }}</option>
            ...
            <div class="flex flex-col items-center mb-4">
                <label class="font-bold text-lg" for="remark">{{ __('form.remark') }}</label>
                <textarea class="w-[80%]" id="remark" name="remark" rows="2" class="form-control"></textarea>
            </div>
            <div class="flex flex-col items-center mb-4" x-data="{ rating: 5 }">
                <label class="font-bold text-lg" for="rating">{{ __('form.rating') }}</label>
                    ...
                    @for ($i = 1; $i <= 5; $i++)
                        <label>
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 512 512"
                                :class="rating >= {{ $i }} ? 'fill-yellow-400' : 'fill-gray-300'"
            ...
            <button type="submit"
                class="text-lg bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded">{{ __('form.submit') }}</button>
        </form>
    </x-slider-with-border-tablet>
</x-app-layout>
